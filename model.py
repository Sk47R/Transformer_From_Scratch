...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_model is not divisible by h"  # ASSURING d_model IS DIVISIBLE BY h
        ...
        self.w_o = nn.Linear(d_model, d_model) # W0 MATRIX
        self.dropout = nn.Dropout(dropout)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):
        '''
            src_mask is used to mask the padding tokens in the input
        '''
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        '''
            src_mask is for the input from the encoder
            tgt_mask is for the input of the decoder
        '''
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff:int = 2048) -> Transformer:
    ...
    # CREATE THE POSITIOINAL ENCODING LAYERS
